    ...
    public class SelectableSerializeReferenceAttributeDrawer : PropertyDrawer {
        ...
        private class PropertyData {
            public PropertyData(SerializedProperty property) {
                ...
                DerivedTypes = TypeCache.GetTypesDerivedFrom(fieldType).Where(x => !x.IsAbstract && !x.IsInterface).ToArray();
            ...
            private static Assembly GetAssembly(string name) { return AppDomain.CurrentDomain.GetAssemblies().SingleOrDefault(assembly => assembly.GetName().Name == name); }
        }
    }
#endif
}