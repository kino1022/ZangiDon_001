...
namespace Teiwas.Script.Spell.Manager {
    public abstract class ASpellManager<Slot, Instance> : SerializedMonoBehaviour, ISpellManager<Slot, Instance> where Slot : ISpellSlot<Instance> where Instance : ISpellInstance {
        ...
        protected KeyValuePair<int, Slot> GetEmptySlot() {

            foreach(var slot in m_spells) {
                if (slot.Value.IsEmpty is true) return slot;
            }

            throw new KeyNotFoundException();
        }

        public virtual void Set(Instance instance) {

            if(IsFull) return;

            var slot = GetEmptySlot();

            slot.Value.SetInstance(instance);
        }

        public virtual void Remove(int index) {
            try {
                var slot = Spells[index];
                slot.RemoveInstance();
            }
